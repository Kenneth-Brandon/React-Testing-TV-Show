...
test('Renders Dropdown Menu and allows the selection and rendering of the Menu Items', async () => {
  mockFetchShow.mockResolvedValueOnce(episodesData);

  const { queryAllByTestId, findByText, queryAllByRole } = render(<App />);

  const dropdown = await findByText(/select a season/i);
  userEvent.click(dropdown);
  const options = queryAllByRole(/option/i);
  expect(options).toHaveLength(1); // if you put total of the TV Shows 4 seasons if fails
